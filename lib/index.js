 ...
 *   keyTimeout: number ≤ 3600 (seconds)
 * }
...
function logglyAppender(config, layout) {
    const client = new YC(config.serviceAccountID, config.keyID, config.keyData, config.destination, config.timeout?config.timeout:1000, config.keyTimeout?config.keyTimeout:3600);
