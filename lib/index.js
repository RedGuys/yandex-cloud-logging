...
function ycAppender(config, layout) {
    if (!config.serviceAccountID || !config.keyID || !config.keyData || !config.destination) {
        throw new Error("Invalid Yandex Cloud logging configuration. Required fields: serviceAccountID, keyID, keyData, destination.");
    }
    const client = new YC(
        ...
        config.resourceId,
        typeof config.setCategoryAsResourceType === "boolean" ? config.setCategoryAsResourceType : true
    );
